...
/* To create new interface */
#include <fcntl.h> // O_RDWR defined
#include <sys/socket.h> // field 'ifru_addr' has incomplete type 'sockaddr' error
#include <linux/if.h>
#include <linux/if_tun.h>
#include <sys/ioctl.h> // ioctl() error
#include <unistd.h> //close() error
/***************************/
...
int tun_alloc(char *dev)
{
     ...
     system("iwconfig | grep wow");

     /* new interface Lin up */
     int sockfd;
     ...
     /********************************/
     system("ifconfig | grep wow");
     return fd;
}

int write_packet_wow(pcap_t* p_out, struct pcap_pkthdr *pkh, unsigned char *h80211 )
{
    int n;
    ...
    n = pkh->caplen;
    printf("buf01: %02x %02x %02x %02x \n",buffer[0],buffer[1],buffer[2],buffer[3]);
    // if( pcap_inject(p_out, buffer, n) < 0 ){
    if( pcap_sendpacket(p_out, buffer, n) < 0 ){
...
int main( int argc, char *argv[] )
{
    ...
    while( 1 )
    {
        ...
        int option = getopt_long( argc, argv, "lb:k:e:o:p:w:c:i:H",
        ...
        switch( option )
        {
            ...
            case 'i' :
                ...
                memset(  opt.interface, 0, sizeof( opt.interface ) );
                strncpy( opt.interface, optarg, sizeof( opt.interface ) - 1 );

                printf("opt.interface: %s\n", opt.interface);
                break;
...
usage:
    ...
    printf("Let's get it stared !\n");
    char *dev = opt.interface;
    printf("%d\n", tun_alloc(dev));
    char errbuf[PCAP_ERRBUF_SIZE];

    printf("Debug %s\n", argv[optind]);
    pcap_t* p_in = pcap_create(argv[optind], errbuf);
    pcap_set_rfmon(p_in, 1);
    pcap_set_promisc(p_in, 1); /* Capture packets that are not yours */
    pcap_set_snaplen(p_in, 65535); /* Snapshot length */
    pcap_set_timeout(p_in, 1000); /* Timeout in milliseconds */
    pcap_activate(p_in);
    /* handle is ready for use with pcap_next() or pcap_loop() */

    pcap_t *p_out = pcap_open_live(dev, 65536, 0, 1, errbuf);
    if (p_out == NULL){
    ...
    while( 1 )
    {
        ...
        /* read one packet */
        if ( pcap_next_ex(p_in, &pkh, &packet_ptr) < 0 ) {
            printf("pcap_next_ex error\n");
            return -1;
        }

        if ((linktype = pcap_datalink(p_in)) == PCAP_ERROR_NOT_ACTIVATED){
        ...
        printf("buffer %02x %02x %02x %02x\n",packet_ptr[0], packet_ptr[1], packet_ptr[2], packet_ptr[3]);
        printf("buffer2 %02x %02x %02x %02x\n",packet_ptr[4], packet_ptr[5], packet_ptr[6], packet_ptr[7]);
        printf("buffer3 %02x %02x %02x %02x\n",packet_ptr[8], packet_ptr[9], packet_ptr[10], packet_ptr[11]);
        ...
        if( h80211[z] != h80211[z + 1] || h80211[z + 2] != 0x03 )
        {
            ...
            if( ( h80211[z + 3] & 0x20 ) == 0 )
            {
                ...
                //    break;
                if( write_packet_wow(p_out, pkh, h80211) != 0 )
                    break;
            }
            else
            {
